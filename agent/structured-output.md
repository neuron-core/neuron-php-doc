...
Validation rules allows Neuron to resend the generation request to the LLM multiple times with a detailed report of what was wrong if one or more properties are not valid, until it reaches the [maxRetries](structured-output.md#max-retries) value.
